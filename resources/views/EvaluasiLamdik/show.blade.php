    ...
    @else text-secondary @endif">
            ...
            @else
                <div class="alert alert-warning text-center">
                    Auditor Belum Mengisi Hasil AMI
                </div>
    ...
    <script>
        ...
        function renderSyaratBody(su, prefix) {
            ...
            if (su.nomor == 1) {
                ...
                if ((d.NDTPS ?? 0) > 0) {
                    html +=
                        `<div class="mt-2 w-100"><small class="text-muted d-block">3 Tahun: Total = S4+S3+S2+S1+INT = <strong>${d.total3 ?? 0}</strong> &rarr; ${(d.persen3 ?? 0).toFixed(1)}%</small>`;
                    html +=
                        `<small class="text-muted d-block">5 Tahun: Total = S2+S1+INT = <strong>${d.total5 ?? 0}</strong> &rarr; ${(d.persen5 ?? 0).toFixed(1)}%</small></div>`;
                }
            }
            html += '</div></div>';
