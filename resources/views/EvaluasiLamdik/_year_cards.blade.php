...
<style>
    ...
    {{-- Add Year Card — same grid slot --}} .add-year-card {
        background: white;
        border-radius: 24px;
        padding: 26px;
...
<div class="audit-grid">
    @foreach ($tahunList as $th)
        ...
        <div class="audit-card">
            ...
            <a href="{{ $routeParamName ? route($routeName, [$routeParamName => $routeParamValue, 'tahun' => $th]) : route($routeName, ['tahun' => $th]) }}"
                class="btn-audit">
    ...
    @if (!isset($showAddYear) || $showAddYear)
        <div class="add-year-card" id="addYearCard" onclick="toggleAddYear()" hidden>
        ...
        <div class="add-year-form-inline" id="addYearForm" style="display:none;">
            <form
                action="{{ $routeParamName ? route($addYearRouteName, $addYearRouteValue) : route($addYearRouteName) }}"
                method="GET" id="addYearFormTag">
                <div class="add-year-input-group">
                    <input type="number" name="tahun" placeholder="cth: {{ now()->year }}" min="2000"
