    ...
    <form action="{{ route('user.store') }}" method="POST">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" class="form-control @error('homebase')
                is-invalid
            @enderror"
                id="homebase" name="homebase" placeholder="Masukkan Nama Dokumen" value="{{ old('homebase') }}">
            @error('homebase')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" class="form-control @error('ketua')
                is-invalid
            @enderror"
                id="ketua" name="ketua" placeholder="Masukkan Nama Dokumen" value="{{ old('ketua') }}">
            @error('ketua')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group mb-3">
            ...
            <input type="email" class="form-control @error('email')
                is-invalid
            @enderror"
                id="email" name="email" placeholder="Masukkan Nama Dokumen" value="{{ old('email') }}">
            @error('email')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group mb-3">
            ...
            <select class="form-select @error('role') is-invalid @enderror" name="role" id="role">
                ...
                <option value="pimpinan" {{ old('role') == 'pimpinan' ? 'selected' : '' }}>Pimpinan Fakultas</option>
