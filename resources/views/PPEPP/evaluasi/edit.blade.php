...
@section('title', 'Penetapan | Edit')

@section('content')

    <h3>Edit Laporan Evaluasi</h3>
    <p class="text-danger my-3">*Wajib</p>
    <form action="{{ route('evaluasi.update', $data->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group mb-3">
            ...
            <select class="form-select @error('aspek') is-invalid @enderror" name="aspek" id="aspek">
                ...
                <option value="Pendidikan" {{ old('aspek', $data->aspek) == 'Pendidikan' ? 'selected' : '' }}>Pendidikan
                </option>
                <option value="Penelitian" {{ old('aspek', $data->aspek) == 'Penelitian' ? 'selected' : '' }}>Penelitian
                </option>
                <option value="Pengabdian" {{ old('aspek', $data->aspek) == 'Pengabdian' ? 'selected' : '' }}>Pengabdian
                </option>
            </select>
            @error('aspek')
        ...
        <div class="form-group mb-3">
            ...
            <select class="form-select @error('jenis_laporan') is-invalid @enderror" name="jenis_laporan"
                ...
                <option value="AMI" {{ old('jenis_laporan', $data->jenis_laporan) == 'AMI' ? 'selected' : '' }}>AMI
                </option>
                <option value="Monev_jurusan"
                    {{ old('jenis_laporan', $data->jenis_laporan) == 'Monev_jurusan' ? 'selected' : '' }}>Monev Jurusan
                </option>
                <option value="Survey" {{ old('jenis_laporan', $data->jenis_laporan) == 'Survey' ? 'selected' : '' }}>Survey
                </option>
            </select>
            @error('jenis_laporan')
                <div class="invalid-feedback">{{ $message }}</div>
