...
@section('content')
    ...
    <script>
        window.jurusanData = {
            matrik: @json($jurusanMatrikJson),
            subItems: @json($jurusanSubItemsJson)
        };
    </script>
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="ami-tab" data-bs-toggle="tab" data-bs-target="#ami" type="button" role="tab"
                aria-controls="ami" aria-selected="false">Hasil AMI</button>
        </li>
    </ul>
    ...
    <div class="tab-content mt-3" id="myTabContent">
                        ...
                        <div class="nav-grid" id="nav-container">
                            ...
                            @foreach ($data as $item)
                                ...
                                <button class="nav-grid-btn nav-item-btn {{ $borderClass }}"
                                    ...
                                    data-temuan="{{ $item->myTemuan ?? '' }}" data-saran="{{ $item->mySaran ?? '' }}"
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="d-flex flex-wrap gap-2 mt-2">
                                            @foreach ($varList as $v)
                                                <span
                                                    class="badge bg-light text-dark border px-3 py-2 fs-6">{{ $v['var'] }}
                                                    = {{ $v['val'] }}</span>
                                            @endforeach
                                        </div>
        ...
        <div class="tab-pane fade" id="ami" role="tabpanel" aria-labelledby="ami-tab">
            {{-- Hasil AMI --}}
            <div class="card shadow-sm">
                ...
                <div class="card mb-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <p>
                                    <strong>Auditor I:</strong>
                                    {{ $auditor->get(0)?->user?->name ?? '-' }}
                                </p>

                                <p>
                                    <strong>Auditor II:</strong>
                                    {{ $auditor->get(1)?->user?->name ?? '-' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <textarea id="catatan_umum" class="form-control input-header" maxlength="200" rows="3"
                                placeholder="Isi catatan umum maksimal 200 karakter..." {{ $isSubmitted ? 'disabled' : '' }}>{{ old('catatan_umum', optional($auditHeader)->catatan_umum ?? '') }}</textarea>
                        </div>
                ...
                <style>
                    /* Animasi berkedip lembut untuk menarik perhatian pengguna */
                ...
                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        const inputs = document.querySelectorAll('.input-header');
                        const btnSimpan = document.getElementById('btn-simpan-header');
                        if (!btnSimpan) return; // AMI sudah disubmit
                        const statusMarker = document.getElementById('status-marker');
                        const btnText = document.getElementById('btn-text');

                        // 1. Simpan nilai awal data saat halaman pertama dimuat
                        inputs.forEach(input => {
                            input.setAttribute('data-original', input.value);

                            // Beri event listener saat pengguna mengubah isi input/textarea
                            input.addEventListener('input', function() {
                                let adaPerubahan = false;

                                // Cek apakah ada salah satu input yang nilainya berbeda dari nilai asli
                                inputs.forEach(i => {
                                ...
                                if (adaPerubahan) {
                                    // Nyalakan tanda "Perlu Simpan" jika ada perubahan
                                    btnSimpan.classList.remove('btn-success');
                                    btnSimpan.classList.add('btn-warning', 'btn-perlu-simpan');
                                    statusMarker.classList.remove('d-none');
                                    btnText.innerText = 'Simpan Perubahan!';
                                } else {
                                    // Kembalikan ke normal jika pengguna membatalkan ketikannya (mengembalikan ke teks asli)
                                    resetTombol();
                                }
                            });
                        });

                        // 2. Fungsi untuk mengembalikan tombol ke status awal (Hijau)
                        function resetTombol() {
                            btnSimpan.classList.remove('btn-warning', 'btn-perlu-simpan');
                            btnSimpan.classList.add('btn-success');
                            statusMarker.classList.add('d-none');
                            btnText.innerText = 'Simpan';

                            // CRUCIAL: Perbarui nilai original dengan nilai yang baru setelah disimpan
                            inputs.forEach(i => i.setAttribute('data-original', i.value));
                        }

                        // 3. Aksi saat tombol Simpan diklik
                        btnSimpan.addEventListener('click', function() {
                            // --- NOTE ---
                            // Jika kamu menggunakan AJAX untuk menyimpan data, letakkan fungsi resetTombol() 
                            // ini di dalam blok 'success' setelah server berhasil merespon.
                            // ------------

                            // Jalankan fungsi reset kembali ke awal
                            resetTombol();

                            // Opsional: Beri feedback singkat kalau dat
                        });
                    });
                </script>
                ...
                @foreach ($kriteria as $itemAmi)
                    @if ($currentKriteriaAmi !== $itemAmi->name)
                        @php
                            $currentKriteriaAmi = $itemAmi->name;
                            $savedKriteria = $auditKriterias->firstWhere('kriteria_id', $itemAmi->id);
                        @endphp
                        ...
                        <div class="card mb-4 shadow-sm card-kriteria-container">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <label class="form-label">Temuan & Catatan Hasil Audit</label>
                                    <textarea class="form-control input-kriteria" rows="4" name="temuan[{{ $itemAmi->id }}]"
                                        placeholder="Masukkan temuan {{ $currentKriteriaAmi }}" {{ $isSubmitted ? 'disabled' : '' }}>{{ old('temuan.' . $itemAmi->id, $savedKriteria->temuan ?? '') }}</textarea>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Saran dan Rekomendasi</label>
                                    <textarea class="form-control input-kriteria" rows="4" name="rekomendasi[{{ $itemAmi->id }}]"
                                        placeholder="Masukkan saran {{ $currentKriteriaAmi }}" {{ $isSubmitted ? 'disabled' : '' }}>{{ old('rekomendasi.' . $itemAmi->id, $savedKriteria->rekomendasi ?? '') }}</textarea>
                                </div>
                ...
                <style>
                    /* Animasi berkedip lembut untuk kriteria yang berubah */
                    @keyframes pulse-kriteria {
                        0% {
                            transform: scale(1);
                            box-shadow: 0 0 0 0 rgba(255, 193, 7, 0.7);
                        }
                    ...
                    .btn-perlu-simpan-kriteria {
                        animation: pulse-kriteria 1.5s infinite;
                    }
                </style>
                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        const kriteriaInputs = document.querySelectorAll('.input-kriteria');

                        // 1. Catat nilai awal bawaan database ke dalam memori atribut HTML
                        kriteriaInputs.forEach(input => {
                            input.setAttribute('data-original', input.value);

                            // 2. Pantau ketikan pengguna di setiap textarea kriteria
                            input.addEventListener('input', function() {
                                // Cari card induk tempat textarea ini berada
                                const cardContainer = this.closest('.card-kriteria-container');
                                const btnSimpan = cardContainer.querySelector('.btn-simpan-kriteria');
                                if (!btnSimpan) return; // AMI sudah disubmit
                                const statusMarker = cardContainer.querySelector('.status-marker');
                                const btnText = cardContainer.querySelector('.btn-text');

                                // Cek semua input yang ada DI DALAM CARD INI SAJA
                                const siblingInputs = cardContainer.querySelectorAll('.input-kriteria');
                                let adaPerubahan = false;

                                siblingInputs.forEach(si => {
                                    if (si.value !== si.getAttribute('data-original')) {
                                        adaPerubahan = true;
                                    }
                                });

                                if (adaPerubahan) {
                                    // Ubah tombol kriteria aktif menjadi kuning berkedip
                                    btnSimpan.classList.remove('btn-success');
                                    btnSimpan.classList.add('btn-warning', 'btn-perlu-simpan-kriteria');
                                    statusMarker.classList.remove('d-none');
                                    btnText.innerText = 'Simpan Perubahan!';
                                } else {
                                    // Kembalikan ke hijau jika tulisan dikembalikan ke semula
                                    resetTombolKriteria(cardContainer);
                                }
                            });
                        });

                        // 3. Fungsi untuk mengembalikan tombol kriteria tertentu ke asal
                        function resetTombolKriteria(cardContainer) {
                            const btnSimpan = cardContainer.querySelector('.btn-simpan-kriteria');
                            const statusMarker = cardContainer.querySelector('.status-marker');
                            const btnText = cardContainer.querySelector('.btn-text');
                            const siblingInputs = cardContainer.querySelectorAll('.input-kriteria');

                            btnSimpan.classList.remove('btn-warning', 'btn-perlu-simpan-kriteria');
                            btnSimpan.classList.add('btn-success');
                            statusMarker.classList.add('d-none');
                            btnText.innerText = 'Simpan';

                            // Perbarui cache nilai original-nya dengan nilai baru yang berhasil disimpan
                            siblingInputs.forEach(si => si.setAttribute('data-original', si.value));
                        }

                        // 4. Deteksi klik pada tombol simpan kriteria
                        const btnSimpanKriterias = document.querySelectorAll('.btn-simpan-kriteria');
                        btnSimpanKriterias.forEach(btn => {
                            btn.addEventListener('click', function() {
                                const currentCard = this.closest('.card-kriteria-container');

                                // --- INTEGRASI AJAX ---
                                // Jika kamu menggunakan AJAX untuk menyimpan data kriteria, 
                                // jalankan fungsi 'resetTombolKriteria(currentCard)' ini tepat di dalam block 'success' AJAX.
                                // ----------------------

                                // Jalankan reset kembali ke awal
                                resetTombolKriteria(currentCard);
            ...
            <script>
                ...
                // Simpan per kriteria
                document.querySelectorAll('.btn-simpan-kriteria').forEach(function(btn) {
                    btn.addEventListener('click', function() {
                        console.log("TERKLIK")
                        ...
                        fetch(`/audit/${jurusan_id}/kriteria`, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    temuan: document.querySelector(
                                        `textarea[name="temuan[${kriteriaId}]"]`).value,
                                    rekomendasi: document.querySelector(
                                        `textarea[name="rekomendasi[${kriteriaId}]"]`).value,
                                })
                            })
                            .then(res => res.json())
                            .then(() => {
    ...
    <script>
        ...
        function saveCurrentForm() {
            ...
            const saran = (formData.get('saran') || '').trim();
            const linkBukti = (formData.get('link_bukti') || '').trim();
            if (nilaiTotal == 0 && !temuan && !saran && !linkBukti) return Promise.resolve();
        ...
        Prmin = Persentase responden minimum
            ...
            <pre class='harkat_penskoran'>${harkat_penskoran}</pre>`);
                ...
                // Inject two-column answer wrapper & switch container
                container.insertAdjacentHTML('beforeend', `
                    <div class="row mt-3" id="answer-row">
                        <div class="col-md-6 border-end pe-2">
                            <h6 class="text-muted mb-2"><i class="bi bi-pencil-square me-1"></i>Isian Auditor</h6>
                            <div id="auditor-col"></div>
                        </div>
                        <div class="col-md-6 ps-2">
                            <h6 class="text-muted mb-2"><i class="bi bi-eye me-1"></i>Jawaban Jurusan</h6>
                            <div id="jurusan-content-body">
                                <p class="text-muted fst-italic">Memuat...</p>
                            </div>
                        </div>
                    </div>
                `);
                container = document.getElementById('auditor-col');
                ...
                if (subItems && subItems.length > 0) {
                    subItems.forEach(item => {
                        ...
                        // Skip all elemen 48 sub-items — rendered as table
                        if (isElemen48) return;
        ...
        `);
            ...
            <div class="mb-3 mt-3">
                ...
                <input type="hidden" name="jawaban" id="jawaban_hidden" value="${jawaban}">`);
                    ...
                    container.insertAdjacentHTML('beforeend', renderTKiTable48());

                    container.insertAdjacentHTML('beforeend', `
                <table class="table table-sm table-bordered mt-2 mb-0 bg-light" id="auto-skora48-table">
                    <thead><tr><th colspan="2" class="text-center">Perhitungan TKi</th></tr></thead>
                    <tbody id="tki-rows48"></tbody>
                    <tr><td><strong>Σ TKi</strong></td><td id="cv-sum48" class="fw-bold">-</td></tr>
                ...
                // =======================
                // 🔥 LINK BUKTI JURUSAN + TEMUAN + SARAN (full-width)
                // =======================
                const formEl = document.getElementById('kriteriaForm');
                const jurusanLink = (window.jurusanData.matrik[id_matriks_led] || {}).link_bukti || '';
...
`);
                ...
                // Render jawaban jurusan di kolom kanan — clone + recompute dengan nilai jurusan
                const jurusanClone = document.getElementById('auditor-col').cloneNode(true);
                const jData = window.jurusanData;
                const jMatrik = jData.matrik[btn.dataset.id] || null;
                const jSubVals = jData.subItems[btn.dataset.id] || {};
                const jContainer = document.getElementById('jurusan-content-body');

                if (jMatrik || Object.keys(jSubVals).length > 0) {
                    // Hapus heading auditor
                    const h6 = jurusanClone.querySelector('h6');
                    if (h6) h6.remove();

                    // Isi radio sesuai jawaban jurusan
                    if (jMatrik) {
                    ...
                    // Isi variabel sesuai jurusan
                    jurusanClone.querySelectorAll('.variabel-input').forEach(el => {
                        const n = (el.getAttribute('name') || '').match(/variabel\[(\d+)\]/);
                        if (n && jSubVals[n[1]] !== undefined) el.value = jSubVals[n[1]];
                    });

                    // Set kelompok dropdown ke pilihan jurusan (bukan auditor)
                    const jKelompok = sessionStorage.getItem('jurusan_kelompok11') || 'sains';
                    const kelSelect = jurusanClone.querySelector('#kelompok-select');
                    if (kelSelect) kelSelect.value = jKelompok;
                    // Simpan & override kelompok utk recompute jurusan
                    const _savedKelompok = window._kelompok11 || 'sains';
                    window._kelompok11 = jKelompok;

                    // Override document.getElementById & querySelector → clone
                    const _gbi = document.getElementById.bind(document);
                    const _qs = document.querySelector.bind(document);
                    document.getElementById = function(id) {
                        return jurusanClone.querySelector('#' + id) || _gbi(id);
                    };
                    document.querySelector = function(sel) {
                        return jurusanClone.querySelector(sel) || _qs(sel);
                    };

                    // Re-run compute untuk elemen ini pada clone
                    // Simpan nilai total auditor sebelum recompute (karena compute update #nilai_total via fallback)
                    const _ntEl = document.getElementById('nilai_total');
                    const _savedNilaiTotal = _ntEl ? _ntEl.value : 0;
                    if (isElemen7) computeFinal7();
                    else if (isElemen11) computeFinal11();
                    else if (isElemen14) computeFinal14();
                    else if (isElemen15) computeFinal15();
                    ...
                    window._kelompok11 = _savedKelompok;
                    if (_ntEl) _ntEl.value = _savedNilaiTotal;

                    // Override semua badge dengan nilai tersimpan jurusan
                    if (jMatrik) {
                    ...
                    // Buat readonly & hapus name/id
                    jurusanClone.querySelectorAll('input, textarea, select').forEach(el => {
                        el.disabled = true;
                        el.readOnly = true;
                        el.removeAttribute('name');
                    });
                    jurusanClone.querySelectorAll('button').forEach(el => el.remove());
                    jurusanClone.querySelectorAll('[id]').forEach(el => el.removeAttribute('id'));
